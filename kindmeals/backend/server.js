    ...
    const uploadStream = cloudinary.uploader.upload_stream(
      ...
      (error, result) => {
        if (error) {
          ...
          console.log(`✅ Cloudinary upload success for folder "${folder}"!`);
...
// API Routes (only modified image upload parts)
...
// Complete donor registration (modified for Cloudinary)
app.post('/api/donor/register', firebaseAuthMiddleware, upload, async (req, res) => {
  try {
    if (!req.firebaseUid) {
      return res.status(401).json({ error: 'Authentication required' });
    ...
    let profileImageUrl = '';
    if (req.files && req.files['profileImage']) {
      profileImageUrl = await uploadFileToCloudinary(
...
// Complete recipient registration (modified for Cloudinary)
app.post('/api/recipient/register', firebaseAuthMiddleware, upload, async (req, res) => {
  try {
    if (!req.firebaseUid) {
      return res.status(401).json({ error: 'Authentication required' });
...
// Complete volunteer registration (modified for Cloudinary)
app.post('/api/volunteer/register', firebaseAuthMiddleware, upload, async (req, res) => {
  try {
    if (!req.firebaseUid) {
      return res.status(401).json({ error: 'Authentication required' });
    ...
    // Handle driving license image upload to Cloudinary
    let drivingLicenseImageUrl = '';
    if (req.files && req.files['drivingLicenseImage']) {
      drivingLicenseImageUrl = await uploadFileToCloudinary(
...
// Create a new donation (modified for Cloudinary)
app.post('/api/donations/create', firebaseAuthMiddleware, upload, async (req, res) => {
  try {
    if (req.userType !== 'donor') {
      return res.status(403).json({ error: 'Only registered donors can create donations' });
    ...
    // Handle food image upload to Cloudinary
    let foodImageUrl = '';
    if (req.files && req.files['foodImage']) {
      foodImageUrl = await uploadFileToCloudinary(
        req.files['foodImage'][0].buffer,
        'food-donations'
      );
    }
...
// Update donor profile (modified for Cloudinary)
app.put('/api/donor/profile', firebaseAuthMiddleware, upload, async (req, res) => {
  try {
    if (!req.firebaseUid) {
      return res.status(401).json({ error: 'Authentication required' });
    ...
    // Handle profile image update to Cloudinary
    if (req.files && req.files['profileImage']) {
      updates.profileImage = await uploadFileToCloudinary(
        req.files['profileImage'][0].buffer,
        'donor-profiles'
      );
    }
...
// Update recipient profile (modified for Cloudinary)
app.put('/api/recipient/profile', firebaseAuthMiddleware, upload, async (req, res) => {
  try {
    if (!req.firebaseUid) {
      return res.status(401).json({ error: 'Authentication required' });
...
app.put('/api/volunteer/profile', firebaseAuthMiddleware, upload, async (req, res) => {
  try {
    if (req.userType !== 'volunteer') {
      return res.status(403).json({ error: 'Only registered volunteers can update their profile' });
    }

    const volunteerId = req.user._id;
    
    // Handle profile image upload to Cloudinary
    let profileImageUrl = req.user.profileImage; // Keep existing image by default
    if (req.files && req.files['profileImage']) {
      profileImageUrl = await uploadFileToCloudinary(
        req.files['profileImage'][0].buffer,
        'volunteer-profiles'
    ...
    // Handle driving license image upload to Cloudinary
    let drivingLicenseImageUrl = req.user.vehicleDetails?.drivingLicenseImage || '';
    if (req.files && req.files['drivingLicenseImage']) {
      drivingLicenseImageUrl = await uploadFileToCloudinary(
        req.files['drivingLicenseImage'][0].buffer,
        'volunteer-licenses'
    ...
    // Check if hasVehicle value is provided and convert to boolean
    let hasVehicle = req.user.hasVehicle;
    if (req.body.hasVehicle !== undefined) {
      hasVehicle = req.body.hasVehicle === 'true';
    }

    // Prepare update object with current values as defaults
    const updateFields = {
      volunteerName: req.body.volunteerName || req.user.volunteerName,
      volunteeraddress: req.body.volunteeraddress || req.user.volunteeraddress,
      volunteercontact: req.body.volunteercontact || req.user.volunteercontact,
      volunteerabout: req.body.volunteerabout || req.user.volunteerabout,
      profileImage: profileImageUrl,
      hasVehicle,
      updatedAt: new Date()
    };

    // Only update vehicle details if user has a vehicle
    if (hasVehicle) {
      // Get current vehicle details or initialize empty object
      const currentVehicleDetails = req.user.vehicleDetails || {};
      
      updateFields.vehicleDetails = {
        vehicleType: req.body.vehicleType || currentVehicleDetails.vehicleType || '',
        vehicleNumber: req.body.vehicleNumber || currentVehicleDetails.vehicleNumber || '',
        drivingLicenseImage: drivingLicenseImageUrl
      };
    }
    ...
    // Update location if provided
    if (req.body.latitude && req.body.longitude) {
      updateFields.volunteerlocation = {
